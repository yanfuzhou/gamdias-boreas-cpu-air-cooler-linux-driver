...
use crate::protocol0::{build_init_packet, build_temperature_packet, build_fan_packet};
...
impl DeviceManager {
            ...
            if device_info.vendor_id() == 0x1b80 
                ...
                && device_info.interface_number() == 0 {
                    ...
                    if let Ok(dev) = device_info.open_device(&api) {
                        ...
                        self.product_id = device.info.product_id();
    ...
    pub fn display_temperature(&self, temperature: f64, celsius: bool, flashing: bool) -> bool {
        self.send_packet(&build_init_packet(temperature, celsius, flashing))
    }
